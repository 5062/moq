...
`moq-trace` is an opt-in Rust crate with one LTTng-UST provider generated by
the official `lttng-gen-tp` tool and
...
Building the crate on Linux requires LTTng-UST 2.13 or newer, `lttng-gen-tp`,
`pkg-config`, and libclang for the generated Rust FFI bindings.
...
process, not for wall-clock comparison between hosts. The payload timestamp is
kept separately from the CTF event clock because packet boundaries can be
recorded after the corresponding scheduling or parsing work occurred.
